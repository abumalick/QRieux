package net.hilson.qrieux.screenshot

import org.junit.Test
import org.junit.runner.RunWith
import org.robolectric.RobolectricTestRunner
import org.robolectric.annotation.Config
import org.robolectric.annotation.GraphicsMode
import net.hilson.qrieux.ui.ScanResultOverlay
import net.hilson.qrieux.util.QrContentType

// The result screen is the one place where the content and every action have to fit
// a screen only a few hundred dp tall. Each type gets its own capture because the
// number of action buttons differs per type, and that is what pushes them off screen.
// The size is a phone held sideways rather than Robolectric's default screen
// rotated, since the two-pane layout only makes sense at a real landscape width.
@RunWith(RobolectricTestRunner::class)
@GraphicsMode(GraphicsMode.Mode.NATIVE)
@Config(sdk = [34], qualifiers = "w891dp-h411dp-land")
class ScanResultOverlayLandscapeScreenshotTest {

    @Test
    fun url() = captureLandscape("scan-result-url") {
        ScanResultOverlay(
            contentType = QrContentType.Url("https://example.com"),
            onDismiss = {},
        )
    }

    @Test
    fun email() = captureLandscape("scan-result-email") {
        ScanResultOverlay(
            contentType = QrContentType.Email("hello@example.com"),
            onDismiss = {},
        )
    }

    @Test
    fun phone() = captureLandscape("scan-result-phone") {
        ScanResultOverlay(
            contentType = QrContentType.Phone("+15551234567"),
            onDismiss = {},
        )
    }

    @Test
    fun wifi() = captureLandscape("scan-result-wifi") {
        ScanResultOverlay(
            contentType = QrContentType.Wifi(
                ssid = "MyNetwork",
                password = "secret",
                authType = "WPA",
                hidden = false,
            ),
            onDismiss = {},
        )
    }

    @Test
    fun contact() = captureLandscape("scan-result-contact") {
        ScanResultOverlay(
            contentType = QrContentType.Contact(
                fullName = "Jane Doe",
                phone = "+15551234567",
                email = "jane@example.com",
                organization = "Acme Co",
                rawVCard = "BEGIN:VCARD\nVERSION:3.0\nFN:Jane Doe\nTEL:+15551234567\nEMAIL:jane@example.com\nORG:Acme Co\nEND:VCARD",
            ),
            onDismiss = {},
        )
    }

    @Test
    fun text() = captureLandscape("scan-result-text") {
        ScanResultOverlay(
            contentType = QrContentType.Text("Plain text payload"),
            onDismiss = {},
        )
    }
}
